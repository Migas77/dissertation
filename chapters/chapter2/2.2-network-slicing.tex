...
The management of network slices spans across the various parts of the network, including both \acs{3GPP} managed network components, such as the \acf{CN} and \acf{AN}, as well as non-\acs{3GPP} components~\cite{ETSI_MANAGEMENT_ORCHESTRATION_CONCEPTS}. For components outside the \acs{3GPP} domain, such as the \acf{TN}, which supports connectivity between the \acs{CN} and the \acs{AN}, the management system must coordinate with the corresponding external management systems to communicate the network slice requirements. These include \acs{TN} topology requirements and individual \acs{TN} links' \acs{QoS} attributes requirements.

For the purpose of \acs{NSI} management, an \acs{NSI} can be divided into one or more \acfp{NSSI}, each representing the instantiation of a \acf{NSS}. As outlined in Figure \ref{fig:slicing:class:diagram}, a \acs{NSS} represents a group of network functions (physical and/or virtualized) and their corresponding resources that form part or all of the constituents of a network slice~\cite{ETSI_MANAGEMENT_ORCHESTRATION_CONCEPTS}. It may include instances of \acs{CN} and/or \acs{AN} network functions, along with thier associated \acs{TN} requirements, catering to both \acs{3GPP} and non-\acs{3GPP} domains across the \acs{5G} network~\cite{ETSI_MANAGEMENT_ORCHESTRATION_CONCEPTS}.
...
Lastly, it is important to reiterate once again that, when providing an \acs{E2E} communication service, the network may rely on non-\acs{3GPP} parts (e.g. the \acs{TN}) in addition to the network components defined in \acs{3GPP}: the \acs{CN} and \acs{AN}. With this goal, the \acs{3GPP} management system derives each domain's requirements from the customer's service requirements and forwards them to the corresponding domain management systems (both \acs{3GPP} and non-\acs{3GPP}), coordinating the exchange of data between them~\cite{ETSI_MANAGEMENT_ORCHESTRATION_CONCEPTS}. Regarding network slicing, on the \acs{3GPP} side, instantiating an \acs{NSI} with \acs{RAN} and \acs{CN} components may entail defining  each domain's component as a separate \acs{NSSI}, potentially shared across multiple \acsp{NSI}~\cite{3GPP-28.801}. On the non-\acs{3GPP} side, when the \acs{NSI} includes \acsp{VNF}, the \acs{NSSMF}, per request from the \acs{NSMF}, creates the needed \acsp{NSSI}, coordinating with an external management system, namely \acs{NFV-MANO}, to configure and instantiate the required \acsp{VNF} and \acsp{NS}, scaling them as needed for the operations of the \acs{NSI}, taking into account with other \acsp{NSSI}. Additionally, the NSSMF triggers the network management system to handle the application-level configuration of the NFs used by the NSSI, similarly accounting for sharing with other NSSIs~\cite{3GPP-28.801}.
