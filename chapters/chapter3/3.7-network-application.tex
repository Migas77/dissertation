...
\midrule
\parbox[t]{3.9cm}{\acs{NEF} \textit{AsSessionWithQoS}} \acs{API} & FR5, C1, PR1, PR5 & Enforcing \acs{QoS} profile, guaranteeting minimum uplink bitrate for drone/\acs{UE} flow \\
\parbox[t]{3.9cm}{\acs{NEF} \textit{MonitoringEvent}} \acs{API} & FR4, PR4 & Subscribing \acs{UE} location change events (type LOCATION\_REPORTING), notifying the application to trigger a \acs{QoS} update upon drone area transitions \\
\parbox[t]{3.9cm}{\acs{NEF} \textit{AnalyticsExposure}} \acs{API} & PR1 & Monitoring of live throughput, volume and \acs{RTT} for continuous performance validation \\
