...
In the context of the proposed testbed architecture, three \acs{NEF} northbound \acsp{API} are considered to address distinct, yet complementary aspects of network application behavior. The \textit{AsSessionWithQoS} \acs{API} targets \acs{QoS} control, providing applications with the means to establish and manage application sessions with specific \acs{QoS} profiles designed to guarantee, for example, specific bandwidth or latency requirements~\cite{ETSI_TS_129_122_V18_8_0}. Beyond session configuration, this \acs{API} also provides built-in mechanisms to monitor \acs{QoS} conditions and detect degradations, allowing the \acs{AF} to receive notifications when the network can no longer sustain the requested \acs{QoS} targets. For broader network observability that extends beyond \acs{QoS}-specific events, the \textit{AnalyticsExposure} \acs{API} gives developers the ability to query network analytics and subscribe to analytics notifications~\cite{ETSI_TS_129_522_V18_11_0}, covering a wider range of network metrics and conditions such as load levels, service experience, and congestion indicators.

\plan{Change the previous paragraph according to the following conditions:
\begin{enumerate}
    \item Change this to not be so negative: "However, passively relying on the \acs{QoS} session established via this \acs{API} is not sufficient, as undetected violations of network requirements directly risk the correct functioning of the Network Application."
    \item Mention that the AsSessionWithQoS also allows to monitor QoS downgrades
    \item Introduce the other AnalyticsExposureAPI as an alternative for this, but also for a variety of other network metrics/conditions
\end{enumerate}
}
