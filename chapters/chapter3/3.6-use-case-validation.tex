...
\label{chap:proposal:use-case-validation}

To assess the testbed's suitability for the development and validation of vertical applications in an end-to-end scenario, a Network Application will need to be developed and evaluated in the proposed testbed. This will not only demonstrate the testbed’s end-to-end capabilities, but also showcase the added value it brings to the deployment and validation of applications tailored to specific vertical scenarios. Section \ref{sota:networkapplications:usecases} highlighted several possible network applications across distinct vertical industries and scenarios, such as \ac{CAM}, \ac{T&L}, and \ac{PPDR}. Given the testbed's goal of catering to multiple vertical industries, the considered \ac{PoC} targets the \ac{PPDR} and Automotive sectors through a vehicle collision detection and notification use case that bridges the two domains.

This use case is motivated by the need to improve the safety of road users in the moments immediately after an accident. Following a vehicle collision, involved and nearby vehicles and pedestrians remain exposed to the hazards of the crash site, increasing the risk of secondary incidents before emergency services arrive. Mitigating this risk requires detecting incidents quickly and immediately alerting those at and approaching the crash site. In this scenario, a computer vision model analyzes the live video stream from fixed roadside cameras to detect collisions and trigger notifications in real time, requiring close integration with the mobile network to automate detection and alerting.
...
\label{chap:proposal:use-case-validation:requirements}
...
\label{chap:proposal:use-case-validation:requirements:functional}
...
\label{chap:proposal:use-case-validation:requirements:performance}
...
\begin{custenumerate}{C}
    \item There shall exist a \acs{QoS} profile defined with a guaranteed minimum uplink bitrate sufficient to ensure that each encoded video frame is delivered to the processing server within at most 33 ms of its capture (FR1), resulting in a frame delivery rate of at least 30 \acs{FPS}.
\end{custenumerate}

With this pre-condition established, the performance requirements define measurable aspects of the application's operation, restraining the functional requirements specified in Section \ref{chap:proposal:use-case-validation:requirements:functional} to temporal constraints conditioned by the active \acs{QoS} profile and application processing overhead, as follows:
...
\label{chap:proposal:use-case-validation:requirements:nonfunctional}
...
The functional, performance, and non-functional requirements defined in the previous section characterize the system's behavior and constraints. From these emerge a complementary category of requirements that is particularly relevant to the context of this dissertation, centered on a Testbed for Testing Network Exposure Interfaces: the interface requirements. Interface requirements specify how the system is required to interact with external systems (external interfaces) or with other elements within the system (internal interfaces)~\cite{ISORequirementsEngineering}.

More specifically, defining the interface requirements entails specifying the actual northbound \acsp{API} that will be supporting network applications, as well as any other related components pertaining to the exposure and accessibility of said \acsp{API}. With this goal, Table~\ref{tab:poc:api:mapping} maps each of the application requirements defined at Section \ref{chap:proposal:use-case-validation:requirements} to the network interface or component responsible for fulfilling it, showcasing how the \acs{PoC} integrates with the components described in the preceding sections.

\plan{ Detail
\begin{enumerate}
    \item usually AsSessionWithQoS uses references. Should the C1 constraint be there?
\end{enumerate}
}
...
\begin{tabular}{p{3.9cm} p{1.4cm} p{8.1cm}}
\toprule
\textbf{Interface} & \textbf{Req.} & \textbf{Role in the \acs{PoC}} \\
\midrule
\parbox[t]{3.9cm}{\acs{NEF} \textit{AsSessionWithQoS}} & FR5, C1, PR1, PR5 & Enforcing \acs{QoS} profile, guaranteeting minimum uplink bitrate for drone/\acs{UE} flow \\
\parbox[t]{3.9cm}{\acs{NEF} \textit{MonitoringEvent}} & FR4, PR4 & Subscribing \acs{UE} location change events (type LOCATION\_REPORTING), notifying the application to trigger a \acs{QoS} update upon drone area transitions \\
\parbox[t]{3.9cm}{\acs{NEF} \textit{AnalyticsExposure}} & PR1 & Monitoring of live throughput, volume and \acs{RTT} for continuous performance validation \\
\mbox{CAMARA \acs{QoS} Profiles} \acs{API} & C1 & Defining the set of available \acs{QoS} profiles and associated parameters, allowing CAMARA \acs{QoD} \acs{API} to select and enforce the requested profile \\
\mbox{CAMARA \acs{QoD} \acs{API}} & FR5, PR1, PR5 & High-level \acs{API} for enforcing the \acs{QoS} profile, guaranteeting minimum uplink bitrate for drone/\acs{UE} flow and abstracting the underlying \acs{NEF} \textit{AsSessionWithQoS} \acs{API} \\
\mbox{CAMARA Geofencing} \mbox{Subscriptions \acs{API}} & FR4, FR5, PR4 & High-level \acs{API} for subscribing to geofencing events that trigger a \acs{QoS} update as the drone entries a defined geographical area, abstracting \acs{NEF} \textit{MonitoringEvent} \acs{API} (type LOCATION\_REPORTING) \\
\mbox{CAMARA Location} \mbox{Retrieval \acs{API}} & FR4 & High-level \acs{API} for retrieving the drone's current location to determine the active operational area and abstracting the underlying \acs{NEF} \textit{MonitoringEvent} \acs{API} (type LOCATION\_REPORTING)  \\
\mbox{CAMARA Location} \mbox{Verification \acs{API}} & FR4 & High-level \acs{API} for verifying whether the drone is within a specified geographical area and abstracting the underlying \acs{NEF} \textit{MonitoringEvent} \acs{API} (type LOCATION\_REPORTING)  \\
\mbox{CAMARA Application} \mbox{Profiles \acs{API}} & C1 & High-level \acs{API} declaring the application's network requirements as a profile, to be used by the Connectivity Insights Subscriptions \acs{API} \\
\mbox{CAMARA Connectivity} \mbox{Insights Subscriptions \acs{API}} & PR1 & High-level \acs{API} for subscribing to network analytics events for validation procedures, abstracting the underlying \acs{NEF} \textit{AnalyticsExposure} \acs{API} \\
\acs{CAPIF} & NR1, NR3 & Onboarding, discovery, authentication and authorization for all testbed \acsp{API}, acting as a single registration point for the network application \\
\bottomrule
\end{tabular}
\caption{Fulfillment of \acs{PoC} application requirements via network exposure interfaces}
\label{tab:poc:api:mapping}
\end{table}

As shown in Table~\ref{tab:poc:api:mapping}, \acs{CAPIF} establishes a single secure exposure layer, through which a variety of \acs{NEF} and CAMARA northbound \acsp{API} are made accessible to network applications. It also reveals that several application requirements, i.e. FR4, FR5, C1, PR1, PR4 and PR5, are satisfied by both \acs{NEF} and CAMARA interfaces simultaneously. However, it is worth emphasizing that the sets of \acs{NEF} and CAMARA \acsp{API} are not redundant, but alternatives targeting different consumer profiles.

On one hand, the \acs{NEF} interfaces are defined by \acs{3GPP} allowing precise control over network capabilities, such as \acs{QoS} session management, event monitoring and analytics exposure. These interfaces expose network functionalities with a degree of technical specificity that requires a deep understanding of the underlying \acs{5G} architecture, data models and overall knowledge of the broader ecosystem.

On the other hand, CAMARA \acsp{API}, aligned with the objectives defined by the CAMARA project~\cite{CAMARAProject}, provide a higher-level user-friendly abstraction, destined to developers with no telco expertise, allowing them to focus on the behavior of their own network applications rather than the complexities of the underlying network, while still taking advantage of its capabilities. Nevertheless, the CAMARA approach still relies on the same underlying \acs{NEF} interfaces to actually deliver network services, according to the information flows described at Section \ref{chap:proposal:exposure:camara}. In the following section, the end-to-end interaction flow of the proposed application is defined, detailing the sequence of operations performed across the various system components and highlighting the difference between the \acs{NEF} and CAMARA \acsp{API} approaches.
...
\plan{
    ...
    \end{enumerate}
    Include a sequence diagram prioritizing the NEF path. Showcase the added procedural complexity of the NEF implementation flow compared to the CAMARA APIs, in the prespective of the application developer/end-user.
}
