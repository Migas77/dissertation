...
To achieve this goal in the testbed, active probing must be initiated automatically upon \acs{UE} and slice provisioning, rather than issued as a one-off manual request. This requires integration between the probing mechanism and the provisioning workflow, such that probing processes are configured, instantiated, and terminated in alignment with the lifecycle of each network slice and its associated \acsp{UE}, as follows:

\begin{itemize}
    \item \textbf{Network Slice Lifecycle:} When a network slice is provisioned, a probe server is configured and deployed within, or reachable from, that slice's data plane, ready to generate and send synthetic traffic towards probing targets. Depending on the implementation, this prober may be dedicated to that slice or shared across multiple slice instances. Upon deprovisioning, all probing infrastructure associated with it is cleaned up.
    \item \textbf{UE Lifecycle:} After a \acs{UE} is provisioned and attached to the slice, its \acs{IP} or \acs{MAC} address is registered as a probing target and a measurement session is initiated. When the \acs{UE} is deprovisioned, the corresponding session is terminated and the target entry is removed.
\end{itemize}

Together, passive monitoring and active probing form the observation layer that allows the testbed to collect network metrics and expose them to third parties via the aforementioned \acs{NEF} \acsp{API}. As a result, network application developers now possess all the mechanisms necessary to validate their applications and react to the testbed's network conditions.
...
\plan{
    \begin{enumerate}
        ...
        \item Add this somewhere (already well written). Not necessarily in this chapter: Network slicing provides the mechanism through which the \acs{5G} core partitions physical infrastructure into isolated logical networks, each configured to satisfy a specific set of \acs{QoS} targets. For network applications that depend on guaranteed service conditions, network slicing represents the baseline for sustaining the stringent network requirements required by such use cases.~\ref{sota:5gecosystem}
    \end{enumerate}
}
